    ...
    return (
        <section style={{position: 'relative', minHeight: '60vh', width: '100%', clipPath: 'inset(0 0 0 0)', padding:'20px'}}>
            <div className="fixed h-full w-full left-0 top-0">
                <Image src={heroImage} layout="fill" objectFit="cover" sizes="100vw" alt="hero of my web site" priority={true} />
            </div>
            <motion.div animate={{opacity: 1, y: 0}} initial={{opacity: 0, y: -20}} transition={{ease: 'linear', duration: 0.75, delay: 0.5}} className="absolute h-full w-full left-0 top-0 flex flex-col justify-center items-center text-center p-3">
