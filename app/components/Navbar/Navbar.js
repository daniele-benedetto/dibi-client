...
import { FaFacebook, FaInstagram, FaUserAlt, FaUserCircle } from "react-icons/fa";
			...
			<nav className="flex justify-between items-center h-16 bg-white text-black relative shadow-sm p-2 z-20">
				...
				<div className="flex items-center justify-between w-24">
					<div className="relative">
						...
						{totalQty > 0 && (
							<span className="absolute top-0 left-0 -translate-x-1/2 -translate-y-1/2 bg-red-500 rounded-full w-4 h-4 flex items-center justify-center text-white text-xs">
								{totalQty}
							</span>
						)}
					</div>
		...
		<nav className="flex justify-between flex-wrap items-center bg-white text-black relative shadow-sm px-10 ">
			...
			>
				<ul className="flex w-full max-w-3xl justify-between text-md uppercase font-black text-lg relative">
					{navbar.map(item => {
						return (
							<li onMouseEnter={() => hoverItemMenu(item)} className="itemMenu" key={item.id}>
			...
			<div className="flex items-center justify-end w-80 absolute top-10 right-10">
				...
				<div className="relative">
					...
					{totalQty > 0 && (
						<motion.span
							animate={{ scale: 1 }}
							initial={{ scale: 0 }}
							className="absolute -top-2 left-0 -translate-x-1/2 -translate-y-1/2 bg-red-500 rounded-full w-4 h-4 flex items-center justify-center text-white text-xs"
						>
							{totalQty}
						</motion.span>
					)}
				...
				<div className="relative">
					...
					{user && (
						<span
							onClick={() => setUserMenuIsOpen(!userMenuIsOpen)}
							className="flex items-center justify-center rounded-full text-gray-700 text-md cursor-pointer mx-2 underline"
						>
							{email}
						</span>
					)}
					{userMenuIsOpen && (
						<div className="absolute top-full translate-y-3 right-3 bg-white shadow-md rounded-md p-5 w-72 max-w-md z-10">
							<AiOutlineClose
				...
				<div className="flex items-center justify-center">
					<Link
						...
						rel="noreferrer"
					>
						<FaFacebook size={24} className="cursor-pointer mx-2" color="#F68129" />
					</Link>
				</div>
				<div className="flex items-center justify-center">
					<Link href="https://www.instagram.com/cian_frusalia/" target="_blank" rel="noreferrer">
						<FaInstagram size={24} className="cursor-pointer mx-2" color="#F68129" />
					</Link>
				</div>
			</div>
			{searchIsOpen && <Searchbar setSearchIsOpen={setSearchIsOpen} />}
